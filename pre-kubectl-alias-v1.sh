...
KCTL_CONFIG='/path-to/kubectl --kubeconfig=/path-to/kubectl-config'
...
then
  if [ ${2} != 0 ] && [ -z ${3} ] || [ ${3} == "-o" ] || [ ${3} == "grep" ]; then                                       #<------- Start 1
    ##### DEFAULT VALUES #####
    echo "===== USE ${R}"**"${NO} =====>  ${C}PRE:${NO} ${Y}(**)${NO} ===========> ${C}Verb:${NO} ${Y}${VERB}${NO}, ${C}Kind:${NO} ${Y}${KIND}${NO} "
    if [ -z ${3} ] || [ ${3} == "-o" ]; then                                                                                    #<----- Start 2
      if [ -z ${3} ]; then                                                                                                                      #<----- Start 3
        ${KCTL_CONFIG} get ${KIND}
      else
        ${KCTL_CONFIG} get ${KIND} -o wide
      fi                                                                                                                                        #<----- End3
    else
      if [ ! -z ${6} ]; then                                                                                                                    #<----- Start 3
        ${KCTL_CONFIG} get ${KIND} -o wide > /tmp/textoutput
        cat /tmp/textoutput | head -n 1
        cat /tmp/textoutput | grep ${4}
      else
        ${KCTL_CONFIG} get ${KIND} > /tmp/textoutput
        cat /tmp/textoutput | head -n 1
        cat /tmp/textoutput | grep ${4}
      fi                                                                                                                                        #<----- End 3
    fi                                                                                                                  #<- End 1 #<----- End 2
    ...
    if [ ${1} == "get" ] && [ ${3} == "-n" ] || [ ${5} == "grep" ]; then                                                        #<-------- Start 1
      echo "===== USE ${R}"**"${NO} =====>  ${C}PRE:${NO} ${Y}(**)${NO} ===========> ${C}Verb:${NO} ${Y}${VERB}${NO}, ${C}Kind:${NO} ${Y}${KIND}${NO}, ${C}Namespace:${NO} ${Y}${NAMESPACE}${NO} "
      if [ -z ${5} ] || [ ${5} == "-o" ]; then                                                                                          #<----- Start 2
        if [ -z ${5} ]; then                                                                                                                    #<----- Start 3
          ${KCTL_CONFIG} get ${KIND} -n ${NAMESPACE}
        else
          ${KCTL_CONFIG} get ${KIND} -n ${NAMESPACE} -o wide
        fi                                                                                                                                      #<----- End 3
      else
        if [ ! -z ${8} ]; then                                                                                                                  #<----- Start 3
          ${KCTL_CONFIG} get ${KIND} -n ${NAMESPACE} -o wide > /tmp/textoutput
          cat /tmp/textoutput | head -n 1
          cat /tmp/textoutput | grep ${6}
        else
          ${KCTL_CONFIG} get ${KIND} -n ${NAMESPACE} > /tmp/textoutput
          cat /tmp/textoutput | head -n 1
          cat /tmp/textoutput | grep ${6}
        fi                                                                                                                                      #<----- End 3
      fi                                                                                                                        #<- End 1 #<----- End 2
    ##### ALL NAMESPACES #####
      ...
      echo "===== USE ${R}"**"${NO} =====>  ${C}PRE:${NO} ${Y}(**)${NO} ===========> ${C}Verb:${NO} ${Y}${VERB}${NO}, ${C}Kind:${NO} ${Y}${KIND}${NO}, ${C}Namespace:${NO} ${Y}all-namespaces${NO} "
      if [ -z ${4} ] || [ ${4} == "-o" ]; then                                                                                          #<----- Start 2
        if [ -z ${4} ]; then                                                                                                                    #<----- Start 3
          ${KCTL_CONFIG} get ${KIND} --all-namespaces
        else
          ${KCTL_CONFIG} get ${KIND} --all-namespaces -o wide
        fi                                                                                                                                      #<----- End 3
      else
        if [ ! -z ${7} ]; then                                                                                                                  #<----- Start 3
          ${KCTL_CONFIG} get ${KIND} --all-namespaces -o wide > /tmp/textoutput
          cat /tmp/textoutput | head -n 1
          cat /tmp/textoutput | grep ${5}
        else
          ${KCTL_CONFIG} get ${KIND} --all-namespaces > /tmp/textoutput
          cat /tmp/textoutput | head -n 1
          cat /tmp/textoutput | grep ${5}
        fi                                                                                                                                      #<----- End 3
      fi                                                                                                                                #<------ End 2
